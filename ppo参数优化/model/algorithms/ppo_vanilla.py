"""
Vanilla PPO — 标准PPO（单一裁剪率，无差异化裁剪）
与MPD-PPO的区别：所有28维统一ε=0.2，标准min(clip) loss
"""
import os
import numpy as np
import torch
    ...
    def update(self, ppo_epochs=PPO_INNER_EPOCHS):
        ...
        for _ in range(ppo_epochs):
            ...
            for start in range(0, dataset_size, mini_batch_size):
                ...
                mb_adv = advantages[mb_idx]; mb_tgt = targets[mb_idx]
                ...
                mb_adv_clamped = torch.clamp(mb_adv.unsqueeze(1), -1e3, 1e3)
                actor_loss = -torch.min(ratio * mb_adv_clamped, clipped * mb_adv_clamped).mean()
                ...
                mb_tgt_safe = torch.where(torch.isfinite(mb_tgt.unsqueeze(1)),
                                          mb_tgt.unsqueeze(1), torch.zeros_like(mb_tgt.unsqueeze(1)))
        ...
        self._clear_buffers()
        avg_a = actor_loss_sum / max(update_count, 1)
        avg_c = critic_loss_sum / max(update_count, 1)
        return avg_a, avg_c
